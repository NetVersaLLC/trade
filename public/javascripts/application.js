  ...
  $.getJSON('/users.js?la='+latitude+'&lo='+longitude, function (data) {
    ...
    $.each(data, function (i) {
      $(function () {
        ...
        if (isSet(user.aim)) {
          info += '<a href="aim:goim?screenname='+user.aim+'&message=Bitcoin+trade."><img src="/images/aol_64.png" alt="AOL Instant Messenger" title="AIM User: '+user.aim+'" class="icon" /></a>';
        }
        if (isSet(user.skype)) {
          info += '<a href="skype:'+user.skype+'?call"><img src="/images/skype_icon_64x64.png" alt="Skype" title="Skype User: '+user.skype+'" class="icon" /></a>';
        }
        if (isSet(user.yim)) {
          info += '<a href="ymsgr:sendIM?'+user.yim+'"><img src="/images/yahoo_64.png" alt="Yahoo Address" title="YIM: '+user.yim+'" class="icon" /></a>';
        }
        if (isSet(user.jabber)) {
          info += '<a href="xmpp:'+user.jabber+'?message"><img src="/images/jabber_icon_64x64.png" title="Jabber: '+user.jabber+'" alt="Jabber" /></a>';
        }
        info += '</div>';
...
function codeAddress(address, callback) {
  ...
  geocoder.geocode( { 'address': address}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        ...
        longitude = results[0].geometry.location.lon();
        if (callback != null) {
          callback();
        }
      } else {
